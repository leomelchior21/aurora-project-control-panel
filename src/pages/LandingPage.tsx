    ...
    <div className="min-h-screen px-4 py-6 md:px-6">
      <div className="mx-auto flex min-h-[calc(100vh-3rem)] max-w-[1440px] flex-col justify-center gap-8">
        ...
        <div className="mx-auto max-w-4xl text-center">
          <p className="text-[11px] uppercase tracking-[0.34em] text-amber-300">Investigação crítica para estudantes</p>
          <h1 className="mt-4 font-headline text-[clamp(2.4rem,5vw,4.8rem)] leading-none text-white">Onde o sistema falhou?</h1>
          <p className="mx-auto mt-4 max-w-3xl text-base leading-7 text-slate-300">
            Escolha uma cidade brasileira e leia os dados como pistas. Aqui não há solução pronta: só sinais de pressão, desigualdade e dependências escondidas.
          </p>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {cityCollection.map((city, index) => (
            <motion.div key={city.slug} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 * index }}>
              <Link to={`/city/${city.slug}`} className="aurora-panel flex min-h-[280px] flex-col justify-between rounded-[32px] border border-white/10 p-6 transition hover:-translate-y-1 hover:border-white/20">
                <div>
                ...
                <div className="flex items-center justify-between pt-8">
                  ...
                  <span className="text-[11px] uppercase tracking-[0.22em]" style={{ color: city.accent }}>Abrir investigação</span>
                </div>
              </Link>
            </motion.div>
          ))}
